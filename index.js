...
function addArtist(artists) {
  ...
  const farhaan = {
    id: 20,
    name: 'Farhaan Nishtar',
    years: '1997 - 02/11/2022',
    genre: 'Blockchain Engineering',
    nationality: 'American',
    bio: 'I am a nice guy with two suitcases in my hands and a dream in my heart'
  }
  artists.push(farhaan);
  return artists;
}
...
function artistByCountry(artists, nationality){
  /*Your Code Here*/
  const sameCountryArtists = [];
  for (let i = 0; i < artists.length; i++) {
    if (artists[i].nationality === nationality) {
      sameCountryArtists.push(artists.[i].name);
    }
  }
  return sameCountryArtists;
}
